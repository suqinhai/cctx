...
├── utils/                  # 工具模块
│   └── metrics.js         # 业绩指标计算（夏普、回撤等）
├── report/                 # 回测报告输出目录
│   ├── nav.csv            # 净值曲线数据
│   ├── trades.csv         # 成交记录明细
│   ├── rolling_sharpe.csv # 滚动夏普比率
│   └── drawdown_periods.csv # 回撤恢复期记录
├── run.js                  # 主程序入口
├── package.json            # 项目配置
└── README.md               # 项目说明
```
...
| 索提诺比率 | 超额收益 / 下行波动率 | 只考虑亏损风险 |
| 卡尔玛比率 | 年化收益 / 最大回撤 | 单位回撤的收益 |
